...
// 	}
	...
	public static Complexe[] appliqueSur(Complexe[] signal) {
		...
		if (signal.length == 1) {
			// Cas trivial de la FFT d'une valeur unique
			...
			for (int k = 0; k < signal.length / 2; ++k) {
				if (P0[k] == null || P1[k] == null) {

					// Gestion du cas où P0[k] ou P1[k] est nul
					trSignal[k] = new ComplexeCartesien(0, 0);
					trSignal[k + signal.length / 2] = new ComplexeCartesien(0, 0);
				} else {
