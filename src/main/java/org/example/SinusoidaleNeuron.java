...
public class SinusoidaleNeuron {
    ...
    public SinusoidaleNeuron() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            float[] signalSinus = NeuronUtil.genererSignalCarre(44100);
        ...
        // Apprentissage
        System.out.println("Apprentissage du neurone avec des signaux sinusoidaux...");
    ...
    public void evaluateFile(String path) {
        Son sinusSound = new Son(path);
        Complexe[] FTTSinus = NeuronUtil.calculerFFT(sinusSound.donnees());
        float[] SignalSinus = NeuronUtil.normaliserCaracteristiques(NeuronUtil.extraireCaracteristiques(FTTSinus));

        // Évaluation du signal sinus
        System.out.println("\nRésultats pour un signal sinusoidale:");
        this.neurone.metAJour(SignalSinus);
